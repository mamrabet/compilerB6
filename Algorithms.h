...
class LabelResult {
private:
typedef std::map<VirtualExpression*, std::pair<GotoInstruction*, GotoInstruction*>, PhiInsertionTask::IsBefore>
Map;
Map m_map;
...
virtual VirtualTask* clone() const { return new PhiInsertionTask(*this); }
virtual int getType() const { return TTPhiInsertion; }
};
...
public:
PhiInsertionAgenda(const Function& function) { addNewAsFirst(new PhiInsertionTask(function)); }
...
virtual int getType() const { return TTLabelPhiFrontier; }
};
